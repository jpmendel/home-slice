...
class TestPostManagement(TestCase):
    def setUp(self):
        ...
        Post.objects.create(author=user, content="My first post")
        Post.objects.create(author=user, content="My second post")
        Post.objects.create(author=user, content="My third post")
    ...
    def test_create_post_success(self):
        response = self.client.post("/api/posts", {"content": "My fourth post"})
        self.assertEqual(response.status_code, 200)
        try:
    ...
    def test_update_post_success(self):
        response = self.client.put(
            "/api/posts/3", urlencode({"content": "My updated third post"})
        )
        self.assertEqual(response.status_code, 200)
        try:
    ...
    def test_update_post_not_found(self):
        response = self.client.put(
            "/api/posts/4", urlencode({"content": "My updated fourth post"})
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_delete_post_not_found(self):
        response = self.client.delete("/api/posts/4")
        self.assertEqual(response.status_code, 404)
